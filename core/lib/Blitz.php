...
class  BlitzPhp{
    ...
    private  function  _handle_param($param){
        ...
        if($this->foreach_count>0){ //如果当前嵌套大于0，说明if在循环内，直接转换变量
            ...
            if($param=='$_key'||$param=='$_num'){
                ...
                //如果是数组，处理方式不同
                if(preg_match('/(.+?)(\[.+?\])/',$param2,$match)){
                    $param3=$parent_label.'[\''.$match[1].'\']'.$match[2]; //构造数组赋值的形式

                }else{
                    $param3=$parent_label.'[\''.$param2.'\']'; //构造数组赋值的形式
                }

                //有个全局替换的过程
                $param='isset('.$param3.')?'.$param3.':'.$param;//如果数组不存在就使用全局变量的设置
            }
        }
        return $param;
    }
    ...
    public   function  middle_compile($content=''){
        ...
        return  $this->last_compile_content= $this->TN($con);
    }
    ...
    //获取最后一次编译的中间结果 ,php代码
    public  function  get_last_compile(){
        return $this->last_compile_content;
    }
    //设置值
