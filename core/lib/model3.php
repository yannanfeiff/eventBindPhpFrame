...
class MYSQL{
    ...
    public  static  function   connect($config=array()){
        ...
        if (mysqli_connect_errno()) {print_stack(); die("Db Error: database connect failed");}
        //设置默认
    ...
    public  static  function  getLink($config=array(),$type=1){
        if($type==1){
            ...
            $lnk = mysql_connect($config['db_host'], $config['db_user'], $config['db_pwd'])
    ...
    public static function getLastId($config = null){
        ...
        if (!$id){
            $result = self::query('SELECT last_insert_id() as id',$config);
